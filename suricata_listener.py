...
FILES_DIR = os.environ.get("SURICATA_WATCHER_FILES_DIR",
...
class EventHandler(pyinotify.ProcessEvent):
    def process_IN_CREATE(self, event):
        if r.match(event.pathname):
            upload_file_if_unknown = live_config.get(
                'upload_file_if_unknown') == 'True'
            p = Process(
                target=new_file_handler,
                args=(event, suricata_file_handler.handle, cache, memory_cache,
                      upload_file_if_unknown, logger))
            p.start()
...
domain_process = Process(
    target=parse_dns_log,
    args=(LOG_DNS, RE_DNS, suricata_domain_handler.handle, cache, memory_cache,
          logger))
domain_process.daemon = True

http_process = Process(
    target=parse_http_log,
    args=(LOG_HTTP, RE_HTTP, suricata_domain_handler.handle, cache,
          memory_cache, logger))
